...
/**
 * Provide HTTP loanrequest functionality via cURL extensions
 *
 * @author tobyb
 * @since 1.0
 */
class CS_REST_CurlTransport extends CS_REST_BaseTransport {
    ...
    function make_call($call_options) {
        ...
        if(!$response && $response !== '') {
            $this->_log->log_message('Error making loanrequest with curl_error: '.curl_errno($ch),
                get_class($this), CS_REST_LOG_ERROR);
            trigger_error('Error making loanrequest with curl_error: '.curl_error($ch), E_USER_ERROR);
        }
...
class CS_REST_SocketWrapper {
    ...
    function open($domain, $port) {
        ...
        if(!$this->socket) {
            die('Error making loanrequest with '.$errno.': '.$errstr);
            return false;
...
class CS_REST_SocketTransport extends CS_REST_BaseTransport {
    ...
    function make_call($call_options) {
        ...
        $this->_log->log_message('Creating socket to '.$domain.' over '.$protocol.' for loanrequest to '.$path,
            get_class($this), CS_REST_LOG_VERBOSE);
    ...
    function _get_status_code($headers) {
        ...
        $this->_log->log_message('Failed to get HTTP status code from loanrequest headers <pre>'.$headers.'</pre>',
            get_class($this), CS_REST_LOG_ERROR);
        trigger_error('Failed to get HTTP status code from loanrequest', E_USER_ERROR);
    }
